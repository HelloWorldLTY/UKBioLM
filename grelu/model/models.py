...
class EnformerNoPreModel(BaseModel):
    """
    Borzoi model with published weights (ported from Keras).
    """
...
class UKBioBERT_EnformerPretrainedModel(BaseModel):
    """
    Borzoi model with published weights (ported from Keras).
    """
    ...
    def __init__(
        ...
        self.ext_emb_len = ext_emb_len
        self.mlp = nn.Sequential(nn.Linear(self.ext_emb_len, 512), nn.ReLU(), nn.Linear(512, 1))
        self.raw_beta = nn.Parameter(data=torch.randn(1), requires_grad=True)
        self.explain = False
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.explain == False:
            x_subset = x[:,0:1,(x.shape[2] - 768*2):]
            x_new = self.embedding(x[:,:, 0:(x.shape[2] - 768*2)])
            
            x_1 = self.head(x_new)
            x_2 = self.mlp(x_subset).reshape(x_1.shape)
...
class UKBioBERT_BorzoiPretrainedModel(BaseModel):
    """
    Borzoi model with published weights (ported from Keras).
    """
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # print(x.shape)
        if self.explain == False:
            x_subset = x[:,0:1,(x.shape[2] - self.ext_emb_len):]
            x_new = self.embedding(x[:,:, 0:(x.shape[2] - self.ext_emb_len)])
