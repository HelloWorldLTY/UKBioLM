...
class MixPearsonCorrelationLoss(torch.nn.Module):
    ...
    def forward(self, preds: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        loss = 1 - r_tss.mean() + self.mse_loss(preds,targets)

        return loss
        
        
def compute_difference_matrix(tensor):
    """
    Computes pairwise differences between all elements in a (N,1) tensor.
    Returns a (N,N) matrix where element (i,j) = tensor[i] - tensor[j]
    """
    # Expand the tensor to (5,1) and (1,5) to enable broadcasting
    x = tensor
    x_i = x.expand(x.size(0), x.size(0))
    x_j = x.T.expand(x.size(0), x.size(0))
...
class PopulationLoss(torch.nn.Module):
    """Pearson Corr balances between across gene and cell performance"""
    ...
    def forward(self, preds: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        pred_diff = compute_difference_matrix(preds.reshape(-1,1))
        true_diff = compute_difference_matrix(targets.reshape(-1,1))

        l1 = self.mse_loss(pred_diff,true_diff)
        l2 = self.mse_loss(preds,targets)
        loss = l1 + l2
...
class LightningModel(pl.LightningModule):
    ...
    def initialize_loss(self) -> None:
        ...
        if self.train_params["task"] == "regression":
            ...
            elif self.train_params["loss"] == "smooth_l1_loss":
                self.loss = nn.SmoothL1Loss()
                
            elif self.train_params["loss"] == "mse_pearson":
                self.loss = MixPearsonCorrelationLoss()
            elif self.train_params["loss"] == "huberloss":
                self.loss = nn.HuberLoss()
                
            elif self.train_params["loss"] == "grouploss":
                self.loss = PopulationLoss()

            else:
                raise Exception("Regression losses: poisson, poisson_multinomial, MSE, smooth_l1_loss")
    ...
    def initialize_activation(self) -> None:
        ...
        if self.train_params["task"] == "regression":
            ...
            # elif (self.train_params["loss"] == "mse") or (self.train_params["loss"] == "smooth_l1_loss") or (self.train_params["loss"] == "mse_pearson"):
            elif self.train_params["loss"] in ["mse", 'smooth_l1_loss', 'mse_pearson', 'huberloss', 'grouploss']:
                self.activation = nn.Identity()
            else:
                raise Exception("Regression losses: poisson, MSE, smooth_l1_loss")
    ...
    def configure_optimizers(self) -> None:
        ...
        if self.train_params["optimizer"] == "adam":
            ...
            return optim.AdamW(self.parameters(), lr=self.train_params["lr"])
        elif self.train_params["optimizer"] == "muon":
            from muon import Muon
            muon_params = [p for p in self.parameters() if p.ndim >= 2]
            # Find everything else -- these will be optimized by AdamW
            adamw_params = [p for p in self.parameters() if p.ndim < 2]
            adamw_params.extend(self.model.head.parameters())
            adamw_params.extend(self.model.embedding.pointwise_conv.parameters())
            adamw_params.extend(self.model.embedding.conv_tower.parameters())
            # Create the optimizer
            optimizer = Muon(muon_params, lr=1e-2, momentum=0.95,
                             adamw_params=adamw_params, adamw_lr=3e-4, adamw_betas=(0.90, 0.95), adamw_wd=0.01) #default
            return optimizer
        else:
    ...
    def make_train_loader(
        ...
        print("should we shuffle?", self.shuffle_train)
        return DataLoader(
            dataset,
    ...
    def embed_on_dataset(
        ...
        for batch in iter(dataloader):
            ...
            if take_mean:
                emb = self.model.embedding(batch).mean(axis=2).detach().cpu()
            else:
                emb = self.model.embedding(batch).detach().cpu()
            preds.append(emb)
