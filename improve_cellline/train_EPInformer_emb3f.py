...
from EPInformer.models import EPInformer_v2, enhancer_predictor_256bp, EPInformer_v3
...
parser.add_argument('--extra_emb', help='external embeddings', default=False)
parser.add_argument('--extra_emb_len', type=int, help='external embeddings length', default=256)
parser.add_argument('--save_path', default='./new/trained_models_pretrain_RNA_3f_hyenadna')
...
import os
saved_model_path = args.save_path+'{}/'.format(datetime_str)
...
for fi in fold_list:
    ...
    all_ds = utils.promoter_enhancer_dataset(data_folder= './data/', expr_type=expr_type, cell_type=cell, n_extraFeat=n_extraFeat, usePromoterSignal=True, n_enhancers=n_enhancers, hic_threshold=hic_threshold, distance_threshold=distance_threshold, extra_emb=args.extra_emb, include_silencer=args.silence)
    ...
    if use_pretrained:
        ...
        model = EPInformer_v3(n_encoder=n_encoder, pre_trained_encoder=pretrained_convNet.encoder, n_enhancer=n_enhancers, out_dim=64, n_extraFeat=n_extraFeat, device=device, use_emb = args.extra_emb_len, silence=args.silence).to(device)
    else:
        model = EPInformer_v3(n_encoder=n_encoder, pre_trained_encoder=None, n_enhancer=n_enhancers, out_dim=64, n_extraFeat=n_extraFeat, device=device, use_emb=args.extra_emb_len, silence=args.silence).to(device)

    model = model.to(device)
    model.name = model.name.replace('EPInformerV3', args.model_type) + '.' +  cell + '.' + expr_type
    utils.train_emb(model, train_ds, valid_dataset=valid_ds, EPOCHS=n_epoch, model_name = model.name, fold_i=fi, batch_size=batch_size, device=device, saved_model_path=saved_model_path)
    test_df = utils.test_emb(model, test_ds, model_name = model.name, saved_model_path=saved_model_path, fold_i=fi, batch_size=batch_size, device=device)
