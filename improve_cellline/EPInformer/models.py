...
class EPInformer_v2(nn.Module):
    def __init__(self, base_size = 4, n_encoder=3, out_dim=128, head = 4, pre_trained_encoder= None, n_enhancer=50, device='cuda', useBN=True, usePromoterSignal=True, useFeat=True, n_extraFeat=0, useLN=True, silence=False):
        ...
        if silence:
            attn_mask = (~np.identity(self.n_enhancer+1+90-61).astype(bool))
        attn_mask[:, 0] = False
        attn_mask[0, :] = False
...
#         print(enhancers_padding_mask.shape)
#         print(enhancers_padding_mask)
        ...
        if extraFeat is not None:
            if pe_flatten_embed.shape[1] == 90:
                extraFeat = torch.concat([extraFeat, torch.zeros((extraFeat.shape[0], 90-61, extraFeat.shape[2])).cuda()], axis=1)
...
class EPInformer_v3(nn.Module):
    def __init__(self, base_size = 4, n_encoder=3, out_dim=128, head = 4, pre_trained_encoder= None, n_enhancer=50, device='cuda', useBN=True, usePromoterSignal=True, useFeat=True, n_extraFeat=0, useLN=True, use_emb=False, silence=False):
        super(EPInformer_v3, self).__init__()
        self.n_enhancer = n_enhancer
        self.out_dim = out_dim
        self.useFeat = useFeat
        self.usePromoterSignal = usePromoterSignal
        ...
        if pre_trained_encoder is not None:
            self.seq_encoder = pre_trained_encoder
            self.name = 'EPInformerV3.preTrainedConv.{}base.{}dim.{}Trans.{}head.{}BN.{}LN.{}Feat.{}extraFeat.{}enh'.format(base_size, out_dim, n_encoder, head, useBN, useLN, useFeat, n_extraFeat, n_enhancer) 
        else:
            self.seq_encoder = seq_256bp_encoder(base_size=base_size)
            self.name = 'EPInformerV3.{}base.{}dim.{}Trans.{}head.{}BN.{}LN.{}Feat.{}extraFeat.{}enh'.format(base_size, out_dim, n_encoder, head, useBN,useLN, useFeat, n_extraFeat, n_enhancer)
        self.n_encoder = n_encoder
        self.device = device
        ...
        if self.useFeat:
            ...
            if self.use_emb:
                self.pToExpr = nn.Sequential(
                    nn.Linear(self.out_dim+feat_n+use_emb, 128),
                    nn.ReLU(),
                    nn.Linear(128, 128),
                    nn.ReLU(),
                    nn.Linear(128, 1),
    ...
    def forward(self, pe_seq, rna_feat=None, extraFeat=None, emb_feat=None):
        # if enhancers_padding_mask is None:
        enhancers_padding_mask = ~(pe_seq.sum(-1).sum(-1) > 0).bool()
#         print(enhancers_padding_mask)
        pe_embed = self.seq_encoder(pe_seq)
        ...
        if self.useFeat:
            p_embed = torch.cat([p_embed, rna_feat], dim=-1)
        if self.use_emb:
            p_embed = torch.cat([p_embed, emb_feat], dim=-1)
        p_expr = self.pToExpr(p_embed)
        return p_expr, torch.cat(attn_list)